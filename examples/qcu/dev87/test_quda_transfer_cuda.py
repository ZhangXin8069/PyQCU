...
try:
    ...
    from pyqcu.solver import (Checkerboard, QudaCoarseOperator,
                              QcuStrictAssetBinding, QudaMatPCOperator,
                              QudaStrictMultigrid, QudaTransfer)
...
def test_quda_strict_recursive_vcycle_matches_reference_and_arena(depth):
    ...
    expected = _reference_strict_vcycle(
        operators, transfers, full_rhs_cpu, parity=0, smoother_steps=2)

    device = torch.device("cuda")
    full_rhs = full_rhs_cpu.to(device)
    full_out = torch.empty_like(full_rhs)
    params, argv = _strict_params(operators, parity=0, smoother_steps=2)
    set_ptrs = define.set_ptrs.clone()
...
def test_cuda_strict_solver_converges_with_bounded_krylov_arena():
    ...
    with CudaStrictMultigridSolver(
            ...
            restart=6, max_krylov_bytes=11 * vector_bytes) as solver:
        owned_refs = tuple(weakref.ref(tensor) for tensor in (
            solver._arena, solver._coarse_rhs, solver._coarse_out,
            solver.fine_null_vectors))
