...
\begin{figure}[htbp]\centering
\includegraphics[width=0.86\linewidth,keepaspectratio]{dev84_volume.png}
\caption{体积标度：speedup vs L1 随体量平坦转降（统一口径 rs5/cf3e3/cmi200），>2 全体量不可达。数据来源：各体量最优实测。}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.8\linewidth,keepaspectratio]{dev84_volume.png}
\caption{体积标度：统一口径（rs5/cf3e3/cmi200）下 speedup 随体量平坦转降——大格子有利假设证伪。数据来源：四体量实测。}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.86\linewidth,keepaspectratio]{dev84_volume.png}
\caption{体积标度：speedup\_vs\_L1 随体量平坦转降（0.62/0.57/0.70/0.42），$>2$ 全体量不可达。数据来源：各体量最优实测。}
\end{figure}
...
% ================= 5 参考 =================
\section{基线口径 II 实测（vs 同配置多线程 BiStabCG）}
...
配置 & MG & BiStabCG & 比值 \\
\midrule
V100 单卡 & 2.29--2.35 s & 1.99--2.05 s & \textcolor{evidence}{1.13--1.16$\times$} \\
P100$\times$2 双线程 & 3.01--3.06 s & 2.62--2.73 s & 0.85--0.86$\times$ \\
\bottomrule
\end{tabular}
\end{table}

要点：V100 同卡 MG 首次稳定超越 BiCGStab（1.15$\times$，本轮优化净效果）；
P100$\times$2 冗余全局模型+慢卡使双卡反不如单 V100（并行比 0.75--0.78$\times$）；
运行间方差显著，正式验收须取多次中位。
终版微优化（细层 r/x 内核融合 + 范数零拷贝直写）再降 MG\_L1 至 1.83s（$-$11\%）、
MG\_L1/BiStabCG $=1.14\times$——MG 全路径逐项受益。
