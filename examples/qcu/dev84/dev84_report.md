...
## 五、基线口径 II 实测（vs 同配置多线程 BiStabCG，三轮稳定值）

| 配置 | MG | BiStabCG | 比值 |
|---|---|---|---|
| V100 单卡 | 2.29–2.35s | 1.99–2.05s | **1.13–1.16×** |
| P100×2 双线程 | 3.01–3.06s | 2.62–2.73s | 0.85–0.86× |

要点：① V100 同卡对比 MG 首次稳定超越 BiStabCG（1.15×，本轮全部优化的净效果），
但距 2× 仍差一倍；② P100×2 因冗余全局模型+慢卡，双卡反不如单 V100
（并行比 0.75–0.78×）；③ 运行间方差显著（首测 mg 4.26s → 稳定 2.3s，
缓存/时钟状态敏感），正式验收须取多次中位；
④ 终版微优化（细层 r/x 内核融合 + 范数零拷贝直写）再降 MG\_L1 至 **1.83s（−11%）**、
MG\_2L 2.89s——MG\_L1/BiStabCG=1.14×，MG 全路径逐项受益；
⑤ **自适应校正门控**（30 迭代斜率标定 + 12 迭代窗口 ×1.15 因子，连续 2 窗不达标即停用
校正）：MG\_2L **2.893→2.383s（−18%）**，speedup\_vs\_L1 升至 **0.798×**，iters 138→118，
正确性保持——多层不再恒劣于基线，粗空间有效时门控自动保持开启。

## 六、自适应门控扩展实测（dev84_2_2）

| 场景 | 门控前 | 门控后 |
|---|---|---|
| run() V100（rs5/cf3e3） | MG_2L 2.893s | **2.383s（−18%）**, 0.798× |
| multi P100×2（rs5/cf1e4/cmi20） | mg≈3.01–3.06s | **3.173s / BiStabCG 2.608s = 0.82×**（一致性 PASS） |
| run_gcr() FGMRES⊕MG | 5.06–6.30s | 4.395s（门控切除尾段弱预条件步；FGMRES+弱预条件子在连续谱上仍无竞争力） |
...
1. 若必须在统一格子达成 >2：需更换比较基线口径（如 vs 多线程 BiStabCG@P100，历史口径）
   ...
   【体积标度实测（分阶段流水线建成后）】0.09×体量 0.62×、0.29×体量 0.57×、
