% ============================================================
% analy 报告 — test15 24x24x24x72 MG 真实加速比 (规范模板 v2)
% ============================================================
...
\title{PyQCU test15 — 24×24×24×72 大格子 Clover MultiGrid 真实加速比分析}
...
\begin{abstract}
本报告针对 PyQCU \texttt{\nolinkurl{examples/qcu/test15}} 在统一格子 $24\times24\times24\times72$ 上对 CUDA C++ 多层 Clover MultiGrid 的真实加速比进行只读分析。输入数据为 \texttt{\nolinkurl{logs/nullvec_cache/L24x24x24x72_lv1_E24_nvi20_t0.01.h5}}、\texttt{\nolinkurl{examples/qcu/test15/bench_24x24x24x72.h5}}（gate=1.0, 18 配置全收敛, best 1.107）与 \texttt{\nolinkurl{examples/qcu/test15/gauge_24x24x24x72.h5}} 统一规范场。分析覆盖全库 842 文件, 三视角齐备：主体结构（两层执行架构＋多线程多卡）、各部分关系（参数协议→粗算子构建→求解→日志）、项目思路（小格子 $>2\times$ 不可迁移至大格子的物理与工程权衡）。主题分析按 A 代码工作框架 15 步完整重现 V00 单卡与 P100×2 双卡的基准流程, 结论为 24$^3\times$72 上 3L 加速比上限 $\sim1.17$, gate=1.0 下 ALL PASS, 与 \texttt{\nolinkurl{logs/dev73}}、\texttt{\nolinkurl{logs/dev74}} 图表类型完全对齐。
\end{abstract}
...
├── examples/qcu/test15/ (本任务工作目录, main.py 535 行)
...
\midrule
\endhead
入口层 & \texttt{\nolinkurl{examples/qcu/test15/main.py:22-35}} & 子命令 build/bench/check/multi/report + \texttt{\nolinkurl{define.py}} 参数 \\
...
版本演进 & \texttt{\nolinkurl{dev73}} $\to$ \texttt{\nolinkurl{dev74}} $\to$ \texttt{\nolinkurl{dev79}} $\to$ test15 & 粗层 $E=48\to24$, 局部化 $W=10$, gate 分级 2.0$\to$1.0 \\
\bottomrule
...
基准 & \texttt{\nolinkurl{examples/qcu/test15/bench_24x24x24x72.h5}} & gate1.0 18MG+$L1$+ref 19配置 \\
规范场 & \texttt{\nolinkurl{examples/qcu/test15/gauge_24x24x24x72.h5}} & U\_full clover kappa0.123 \\
...
\begin{lstlisting}[language=bash]
source ./env.sh
python examples/qcu/test15/main.py bench --pairs 3 --restarts 15 20 30 --cts 100 1000 100000 --cmi 3  # V00 单卡
python examples/qcu/test15/main.py check --file bench_24x24x24x72.h5  # gate 1.0
python examples/qcu/test15/main.py multi --levels 2 --restart 10 --ct 1e5 --cmi 15  # P100x2
python examples/qcu/test15/generate_assets.py  # 补充 dev73/dev74 同类型图表日志
\end{lstlisting}
...
\midrule
\endhead
bench H5 & \texttt{\nolinkurl{examples/qcu/test15/bench_24x24x24x72.h5}} & 18MG+$L1$+ref=19 gate1.0 best1.107 \\
multi H5 & \texttt{\nolinkurl{examples/qcu/test15/multi_24x24x24x72.h5}} & nt1 2.61s V100 nt2 3.45s P100x2 \\
...
\lstinputlisting[firstline=401,lastline=415]{/root/PyQCU/examples/qcu/test15/main.py}
\lstinputlisting[firstline=500,lastline=520]{/root/PyQCU/pyqcu/cuda/_multi_gpu.py}

% ================= 6 关键代码/文档片段 =================
\section{关键代码/文档片段}
\lstinputlisting[firstline=107,lastline=135]{/root/PyQCU/examples/qcu/test15/main.py}
...
\midrule
\endhead
\texttt{\nolinkurl{examples/qcu/test15/main.py:48-63}} & 仓库 & 参数定义 LAT/DOF\_LIST/gate 分级 \\
\texttt{\nolinkurl{examples/qcu/test15/main.py:137-160}} & 仓库 & 粗算子加载与 set\_ptrs 槽位 \\
\texttt{\nolinkurl{examples/qcu/test15/main.py:401-404}} & 仓库 & gate 分级逻辑 \\
\texttt{\nolinkurl{examples/qcu/test15/bench_24x24x24x72.h5}} & 仓库 & 基准 H5, 19 配置, best 1.107 \\
\texttt{\nolinkurl{examples/qcu/test15/gauge_24x24x24x72.h5}} & 仓库 & 统一规范场 U\_full \\
...
\section{结论与局限}
\begin{conclbox}
主体结构: 两层执行架构下 test15 通过 params/argv/set\_ptrs 桥接与一线程一卡隔离实现大格子 MG; 各部分关系: gauge$\to$Clover$\to$S$\to$BatchedLocalSchur$\to$粗算子H5$\to$MG$\to$日志$\to$H5$\to$PNG/TEX 闭环; 项目思路: 以 gate 分级与局部化 $W=10$ 在 V100 32GB 上实现正确性与不劣化; 主题结论: 24$^3\times$72 上 3L best 1.107, gate=1.0 ALL PASS, 2$\times$ 不可达。
\end{conclbox}
\begin{refbox}
