#!/usr/bin/env python3
"""test15 —— CUDA C++ 多线程版 MultiGrid 真实加速比测试套件（examples/qcu/test15）。
...
统一 gauge 与 nullvec 缓存复用（logs/nullvec_cache + examples/qcu/test15/gauge）。
...
_HERE = os.path.dirname(os.path.abspath(__file__))           # examples/qcu/test15
...
def cmd_build(args):
    ...
    gp = os.path.join(GAUGE_DIR, f'gauge_{_lat_str()}.h5')
...
def cmd_bench(args):
    ...
    out = os.path.join(_HERE, f'bench_{_lat_str()}.h5')
...
def cmd_check(args):
    ...
    import h5py
    path = args.file or os.path.join(_HERE, f'bench_{_lat_str()}.h5')
    with h5py.File(path, 'r') as f:
        gate = float(f['gate'][()])
        entries = []
...
def cmd_multi(args):
    ...
    with h5py.File(os.path.join(_HERE, f'multi_{_lat_str()}.h5'), 'w') as f:
        f.create_dataset('lat', data=[int(x) for x in LAT])
...
def cmd_report(args):
    """打印报告摘要。"""
    import h5py
    for name in (f'bench_{_lat_str()}.h5', f'multi_{_lat_str()}.h5'):
