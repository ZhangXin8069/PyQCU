...
def main() -> int:
    ...
    argv_local = argv.to(dtype=define.dtype(params[define._DATA_TYPE_]).to_real())
    argv_local[define._MASS_] = 0.05
    argv_local[define._ATOL_] = 1e-9
    argv_local[define._SIGMA_] = 0.1
    print(params)
    print(argv_local)
    print(set_ptrs)
    ...
    params[define._SET_PLAN_] = -1
    params[define._PARITY_] = 0
    qcu.applyInitQcu(set_ptrs, params, argv_local)
    ...
    refer_clover_term = dslash.make_clover(
        U=qcu_U, kappa=1 / (2 * argv_local[define._MASS_] + 8))
    ...
    for i in range(10):
        ...
        refer_src = dslash.give_wilson(
            src=qcu_dest, U=qcu_U, kappa=1 / (2 * argv_local[define._MASS_] + 8), with_I=True)+dslash.give_clover(src=qcu_dest, clover_term=refer_clover_term)
