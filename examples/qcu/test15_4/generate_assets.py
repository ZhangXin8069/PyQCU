...
def plot_conv(entries):
    # Enhanced residual plot: 3-panel comprehensive view
    # - 左上: 残差 vs 迭代 (log) 含 ref 与 best 5 MG, 淡化参考线
    # - 右上: 残差 vs  wall time 估计 (t_med/iters * iter)
    # - 下: 迭代数 vs 耗时 散点 + 误差带 + 表格
    # Fix: larger canvas for residual detail (was 12x8 too small for 18 curves)
    ...
    for ax in (ax0,ax1,ax2):
        _style(ax)
    # --- 左上: 残差 vs 迭代 ---
    ax0.set_title("残差 vs 迭代 (c64, 24³×72)", color=INK, fontsize=10)
    ax0.set_xlabel("迭代次数", color=INK2, fontsize=8)
    ax0.set_ylabel("Schur 残差 ||r||", color=INK2, fontsize=8)
    ref_hist = entries[0]['ref_hist']
    # 参考线淡化: 极细虚线, 低 alpha, 置于底层
    ax0.axhline(1e-6, color="#e0e0e0", lw=0.8, ls=":", zorder=0, alpha=0.9)
    ax0.text(0.02, 1.2e-6, "atol=1e-6", color="#a0a0a0", fontsize=6, va="bottom", ha="left", alpha=0.9, zorder=0)
    ax0.plot(range(len(ref_hist)), ref_hist, color=C["green"], lw=1.8, label="BiStabCG ref (148it, 3.20s)", marker="o", ms=2.5, zorder=3, alpha=0.95)
    # 仅展示 best 6 与 worst 2 以避免过度拥挤, 其余淡化
    sorted_e = sorted(entries, key=lambda x: x['speedup_vs_L1'], reverse=True)
    highlight = sorted_e[:6]
    faint = sorted_e[6:12]
    for e in faint:
        ax0.plot(range(len(e['conv_mg'])), e['conv_mg'], color="#e0e0e0", lw=1.0, alpha=0.6, zorder=1)
    for i, e in enumerate(highlight):
        color = SEQ_BLUE[(i*2+2)%len(SEQ_BLUE)]
        ls = "-" if e['levels']==3 else "--"
        # Compact label: omit t_med to reduce width, keep speedup only for best
        lbl = f"L{e['levels']} r{e['restart']} ct{e['ct']:.0e}"
        ax0.plot(range(len(e['conv_mg'])), e['conv_mg'], color=color, lw=1.6, ls=ls, label=lbl, marker="s", ms=2, zorder=3, alpha=0.9)
    ax0.set_yscale("log"); ax0.set_ylim(3e-7, 5e3)
    ax0.set_xlim(0, 155)
    ax0.grid(True, which="both", axis="y", color=GRID, linewidth=0.4, alpha=0.5)
    # 图例置于图外右侧, 避免遮挡
    ax0.legend(fontsize=5.0, frameon=True, loc="center left", bbox_to_anchor=(1.02, 0.5), handlelength=1.2, borderpad=0.3, labelspacing=0.25)
    ax0.grid(True, axis="y", color=GRID, linewidth=0.5, alpha=0.7)
    # --- 右上: 残差 vs 预估 wall time ---
    ax1.set_title("残差 vs 预估耗时 (t_med/iters × iter)", color=INK, fontsize=10)
    ax1.set_xlabel("预估耗时 (s)", color=INK2, fontsize=8)
    ax1.set_ylabel("残差", color=INK2, fontsize=8)
    ax1.axhline(1e-6, color="#e0e0e0", lw=0.8, ls=":", zorder=0, alpha=0.9)
    for e in highlight[:4]:
        it = len(e['conv_mg'])
        t_per_it = e['t_med']/it
        times = [i*t_per_it for i in range(it)]
        color = SEQ_BLUE[(highlight.index(e)*2+2)%len(SEQ_BLUE)]
        ax1.plot(times, e['conv_mg'], color=color, lw=1.6, label=f"L{e['levels']} r{e['restart']}", marker="s", ms=2, zorder=3, alpha=0.9)
    # ref time curve
    ref_t_per = ref_time/len(ref_hist)
    ax1.plot([i*ref_t_per for i in range(len(ref_hist))], ref_hist, color=C["green"], lw=1.8, ls="--", label="BiStabCG", zorder=3, alpha=0.85)
    ax1.set_yscale("log"); ax1.set_ylim(5e-7, 5e3)
    ax1.legend(fontsize=5.0, frameon=True, loc="upper right", handlelength=1.2)
    ax1.grid(True, axis="y", color=GRID, linewidth=0.5, alpha=0.7)
    # --- 下: 迭代-耗时散点 + 误差带 ---
    ax2.set_title("配置对比: 迭代数 vs 中位耗时 (含 3 次测量误差带)", color=INK, fontsize=10)
    ax2.set_xlabel("迭代数 (mg_iters)", color=INK2, fontsize=8)
    ax2.set_ylabel("t_med (s)", color=INK2, fontsize=8)
    # Clear error bands: directly show all 3 actual measurements per config
    for e in entries:
        x = e['mg_iters']
        tl = e.get('t_list', [e['t_med']]*3)
        color = C["blue"] if e['levels']==2 else C["orange"]
        marker = "o" if e['levels']==2 else "s"
        # Plot each actual measurement as separate point (no error bar)
        for y in tl:
            ax2.scatter(x, y, s=28, color=color, marker=marker, alpha=0.85, edgecolors="white", linewidths=0.6, zorder=3)
        # Also show median as larger hollow marker
        ax2.scatter(x, e['t_med'], s=55, facecolors="none", edgecolors=color, linewidths=1.2, marker=marker, zorder=4, alpha=0.9)
        # label best
        if e in highlight[:3]:
            ax2.annotate(f"L{e['levels']} r{e['restart']}", xy=(x,y), xytext=(4,6), textcoords="offset points", fontsize=5, color=INK2, ha="left", va="bottom",
                         arrowprops=dict(arrowstyle="->", color=MUTED, lw=0.8, alpha=0.7))
    # 参考线淡化: L1 与 ref 垂直线
    # 用极淡背景带而非实线
    ax2.axhspan(l1_med*0.98, l1_med*1.02, color=C["green"], alpha=0.08, zorder=0)
    ax2.text(ax2.get_xlim()[1]*0.98, l1_med*1.04, f"L1 {l1_med:.2f}s", color=C["green"], fontsize=6, ha="right", va="bottom", alpha=0.9,
             bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor=C["green"], alpha=0.8))
    ax2.axhspan(ref_time*0.98, ref_time*1.02, color=C["orange"], alpha=0.06, zorder=0)
    ax2.text(ax2.get_xlim()[1]*0.98, ref_time*1.04, f"BiStabCG {ref_time:.2f}s", color=C["orange"], fontsize=6, ha="right", va="bottom", alpha=0.9,
             bbox=dict(boxstyle="round,pad=0.2", facecolor="white", edgecolor=C["orange"], alpha=0.8))
    # 区分 2L/3L 图例 (手动)
    from matplotlib.lines import Line2D
    legend_elements = [Line2D([0],[0], marker='o', color='w', markerfacecolor=C["blue"], markersize=6, label='2L'),
                       Line2D([0],[0], marker='s', color='w', markerfacecolor=C["orange"], markersize=6, label='3L')]
    ax2.legend(handles=legend_elements, fontsize=6, frameon=True, loc="upper left", handletextpad=0.4)
    ax2.grid(True, color=GRID, linewidth=0.5, alpha=0.6)
    fig.suptitle("收敛历史综合 (24³×72, c64) — 迭代/耗时双视角 + 误差带", color=INK, fontsize=11, y=0.98)
    fig.tight_layout(rect=[0,0,1,0.96])
    out=os.path.join(LOG_DIR, f"{TAG}_conv_24x24x24x72_c64.png")
    fig.savefig(out, dpi=220, bbox_inches="tight", facecolor=SURF)
    plt.close(fig)
    print(f"saved {out} (enhanced 3-panel)")
