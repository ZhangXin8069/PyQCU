#!/usr/bin/env python3
"""Baseline: current C++ Clover Multigrid vs BiStabCG reference.

Runs a small config matrix to establish the CURRENT state before any fixes.
Usage: source ./env.sh && python examples/qcu/mg_baseline.py
"""
...
def run(label, Lx, Ly, Lz, Lt, MASS, ATOL, NUM_LEVELS, DOF_LIST, MG_GRID,
        ...
        verbose=True, log_fn="clover_multigrid.log"):
    ...
    # Reference BiStabCG (parity-preconditioned Clover)
    ...
    # Run C++ MG (verbose ON so convergence goes to logs/clover_multigrid.log)
    ...
    conv = []
    log_path = os.path.join(os.path.expanduser("~/PyQCU/logs/dev73"), log_fn)
...
if __name__ == "__main__":
    CONFIGS = [
        ("8x8x8x16_c64_1L",  8, 8, 8, 16, 0.05, 1e-6, 1, [12],    [2,2,2,2], 5, 100, 10.0, define._LAT_C64_),
        ("8x8x8x16_c64_2L",  8, 8, 8, 16, 0.05, 1e-6, 2, [12,24], [2,2,2,2], 5, 100, 10.0, define._LAT_C64_),
        ("8x16x16x16_c64_2L",8,16,16,16,0.05, 1e-6, 2, [12,24], [2,2,2,2], 5, 100, 10.0, define._LAT_C64_),
    ]
    results = []
    for cfg in CONFIGS:
