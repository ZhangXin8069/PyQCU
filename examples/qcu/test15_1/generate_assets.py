...
LOG_DIR = os.path.join(REPO, "logs", "test15_1")
...
def plot_conv(entries):
    ...
    # Fix: legend outside to not cover conv curves (was loc best overlapping)
    ax.legend(fontsize=5.5, frameon=True, loc="upper right", bbox_to_anchor=(1.0, 0.98), ncol=1, handlelength=1.5, borderpad=0.4, labelspacing=0.3)
    fig.tight_layout()
    out=os.path.join(LOG_DIR, f"{TAG}_conv_24x24x24x72_c64.png")
    fig.savefig(out, dpi=150, bbox_inches="tight", facecolor=SURF)
    plt.close(fig)
    print(f"saved {out}")
    # unified TAG naming: single output
