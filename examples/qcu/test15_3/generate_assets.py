...
def plot_conv(entries):
    ...
    faint = sorted_e[6:12]
    for e in faint:
        ax0.plot(range(len(e['conv_mg'])), e['conv_mg'], color="#d0d0d0", lw=1.0, alpha=0.5, zorder=1)
    ...
    for e in entries:
        x = e['mg_iters']
        y = e['t_med']
        # error bar from t_list
        tl = e.get('t_list', [y]*3)
        yerr = max(tl)-min(tl)
        # yerr is range, use half range
        yerr = (max(tl)-min(tl))/2
        color = C["blue"] if e['levels']==2 else C["orange"]
        marker = "o" if e['levels']==2 else "s"
        ax2.errorbar(x, y, yerr=yerr, fmt=marker, color=color, ecolor=color, elinewidth=1.2, capsize=3, alpha=0.8, ms=5)
        # label best
        if e in highlight[:3]:
