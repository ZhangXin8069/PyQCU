...
def run_gemm(
    ...
    def ref_program(A, B):
        ...
        if trans_B:
            B = B.T
        return (A.to(torch.float32) @ B.to(torch.float32)).to(torch.__getattribute__(out_dtype))
...
def test_gemm_fp8fp8f32():
    try:
        from tilelang.contrib.rocm import get_rocm_arch, find_rocm_path
        if get_rocm_arch(find_rocm_path()) == "gfx936":
