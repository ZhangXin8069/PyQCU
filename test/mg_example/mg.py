...
class mg:
    ...
    def near_null_vec(self, P_null_vec_coarse, coarse_dof, coarse_op):
        for i in range(0, coarse_dof):
            # 施密特正交化
            for k in range(0, i):
                P_null_vec_coarse[i, :, :, :] -= cp.vdot(cp.conj(P_null_vec_coarse[i, :, :, :]), P_null_vec_coarse[k, :, :, :])/cp.vdot(
                    cp.conj(P_null_vec_coarse[k, :, :, :]), P_null_vec_coarse[k, :, :, :])*P_null_vec_coarse[k, :, :, :]
            # Ar
            ...
            P_null_vec_coarse[i, :] = P_null_vec_coarse[i, :]/cp.sqrt(
                cp.vdot(cp.conj(P_null_vec_coarse[i, :]), P_null_vec_coarse[i, :]))
            # if i==0:
    ...
    def zeros_like_fermi(self, level):
        fermi_out = cp.random.rand(
            self.mg_ops[level].nx, self.mg_ops[level].ny, self.mg_ops[level].nc*2).view(cp.complex128)
    ...
    def __init__(self, fine_op, n_refine, ifeigen=0):
        ...
        if (ifeigen == 0):
            ...
            for i in range(0, n_refine):
                print("当前层： ", i)
                print("创造近零空间向量...")
                P_null_vec_coarse = cp.random.rand(
                    self.coarse_dof[i], nx, ny, nc*2).view(cp.complex128)
                ...
                ################################# transfer ######################################
                self.mg_ops[i +
                            1].clover = cp.zeros_like(self.mg_ops[i+1].clover)
                # clover
                print("clover:")
                for color in range(0, self.mg_ops[i+1].nc):
                    ...
                    fermi_tmp_Afine = lattice.apply_clover(
                ...
                self.mg_ops[i +
                            1].hopping = cp.zeros_like(self.mg_ops[i+1].hopping)
                print("wilson:")
                for color in range(0, self.mg_ops[i+1].nc):
                    print(color)
                    # xp=even
                    ...
                    self.prolong_c2f(i, fermi_tmp_coarse, fermi_tmp_fine)
                    fermi_tmp_Afine = lattice.apply_hopping_x_p(
                        fermi_tmp_fine, self.mg_ops[i])
                    fermi_tmp_coarse = cp.zeros_like(fermi_tmp_coarse)
                    self.restrict_f2c(i, fermi_tmp_Afine, fermi_tmp_coarse)
                    ...
                    fermi_tmp_coarse[0:-1:2, :, color] = 1
                    self.prolong_c2f(i, fermi_tmp_coarse, fermi_tmp_fine)
                    fermi_tmp_Afine = lattice.apply_hopping_x_m(
                        fermi_tmp_fine, self.mg_ops[i])
                    fermi_tmp_coarse = cp.zeros_like(fermi_tmp_coarse)
                    self.restrict_f2c(i, fermi_tmp_Afine, fermi_tmp_coarse)
                    ...
                    fermi_tmp_coarse[:, 0:-1:2, color] = 1
                    self.prolong_c2f(i, fermi_tmp_coarse, fermi_tmp_fine)
                    fermi_tmp_Afine = lattice.apply_hopping_y_p(
                        fermi_tmp_fine, self.mg_ops[i])
                    fermi_tmp_coarse = cp.zeros_like(fermi_tmp_coarse)
                    self.restrict_f2c(i, fermi_tmp_Afine, fermi_tmp_coarse)
                    ...
                    fermi_tmp_coarse[:, 0:-1:2, color] = 1
                    self.prolong_c2f(i, fermi_tmp_coarse, fermi_tmp_fine)
                    fermi_tmp_Afine = lattice.apply_hopping_y_m(
                        fermi_tmp_fine, self.mg_ops[i])
                    fermi_tmp_coarse = cp.zeros_like(fermi_tmp_coarse)
                    self.restrict_f2c(i, fermi_tmp_Afine, fermi_tmp_coarse)
                ...
                fermi_tmp_coarse_n = lattice.apply_mat(
                    fermi_tmp_coarse, self.mg_ops[i+1])
                self.prolong_c2f(i, fermi_tmp_coarse, fermi_tmp_fine)
                fermi_tmp_Afine = lattice.apply_mat(
                    fermi_tmp_fine, self.mg_ops[i])
