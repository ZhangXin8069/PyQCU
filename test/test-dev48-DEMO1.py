...
class AdaptiveMultigridComplex:
    ...
    def v_cycle(self, A_hierarchy, b_hierarchy, u_hierarchy, grid_sizes, level=0):
        """V-循环"""
        ...
        if current_level > 0:
            ...
            e_coarse = self.v_cycle(
                A_hierarchy, b_hierarchy, u_hierarchy, grid_sizes, level + 1)
    ...
    def solve(self, alpha=1.0, beta=1.0j, func_type='sine'):
        ...
        for i, (nx, ny) in enumerate(grid_sizes):
            ...
            A = self.create_complex_matrix(nx, ny, alpha, beta)
    ...
    def verify_solution(self, solution, alpha=1.0, beta=1.0j, func_type='sine'):
        ...
        # 重新计算矩阵和右端项
        A = self.create_complex_matrix(self.nx, self.ny, alpha, beta)
