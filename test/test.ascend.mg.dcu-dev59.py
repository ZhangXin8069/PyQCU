...
MPI.Init()
print(f"MPI.Is_initialized(): {MPI.Is_initialized()}")
...
_qcu = qcu(lat_size=[lat_x, lat_y, lat_z, lat_t], dtype_list=[torch.complex128, torch.complex64, torch.complex64, torch.complex32], device_list=[
           torch.device('cuda'), torch.device('cuda'), torch.device('cuda'), torch.device('cuda')], dslash='clover', solver='mg', verbose=False)
_qcu.load()
_qcu.init()
_qcu.solve()
_qcu.test()
