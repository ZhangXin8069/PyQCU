...
from pyqcu import _torch, lattice


def bicgstab(b: torch.Tensor, matvec: Callable[[torch.Tensor], torch.Tensor], tol: float = 1e-6, max_iter: int = 1000, x0: torch.Tensor = None, if_rtol: bool = False, if_multi: bool = lattice.give_if_multi(), verbose: bool = True) -> torch.Tensor:
    x = x0.clone() if x0 is not None else _torch.randn_like(b)
    r = b - matvec(x)
    r_norm = _torch.norm(r)
    if if_rtol:
        _tol = _torch.norm(b)*tol
    else:
        _tol = tol
    if verbose:
        print(f"PYQCU::SOLVER::BISTABCG:\n Norm of b:{_torch.norm(b)}")
        print(f"PYQCU::SOLVER::BISTABCG:\n Norm of r:{r_norm}")
        print(f"PYQCU::SOLVER::BISTABCG:\n Norm of x0:{_torch.norm(x)}")
    if r_norm < _tol:
