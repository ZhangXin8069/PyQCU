...
def npu_restrict(local_ortho_null_vecs: torch.Tensor, fine_vec: torch.Tensor, verbose: bool = True) -> torch.Tensor:
    ...
    # [eTt, Z, z, Y, y, X, x]
    _fine_vec = _fine_vec.reshape(-1, Z, z, Y, y, X, x)
    _fine_vec = _fine_vec.permute(
        0, 1, 3, 5, 2, 4, 6)  # [eTt, Z, Y, X, z, y, x]
    _fine_vec = _fine_vec.reshape(e, T, t, Z*Y*X, z*y*x)
    _fine_vec = _fine_vec.permute(0, 1, 3, 2, 4)  # [e, T, Z*Y*X, t, z*y*x]
    _fine_vec = _fine_vec.reshape(e, -1, t, z, y, x)
...
def npu_prolong(local_ortho_null_vecs: torch.Tensor, coarse_vec: torch.Tensor, verbose: bool = True) -> torch.Tensor:
    ...
    _local_ortho_null_vecs = _local_ortho_null_vecs.reshape(
        E, e, -1, t, z, y, x)
    return torch_einsum(
        "EeOtzyx,EO->eOtzyx", _local_ortho_null_vecs, _coarse_vec).reshape(E, e, T, t, Z, z, Y, y, X, x).to(dtype=dtype, device=device)
