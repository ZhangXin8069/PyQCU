    ...
    if (data_type == _LAT_C64_)
    {
        ...
        // }
        {
            ...
            { // _wilson_dslash_no_dag
                ...
                cg_give_dest_o<float><<<set_ptr->gridDim, set_ptr->blockDim, 0,
                                        set_ptr->stream>>>(
                    device_vec2, fermion_in, device_vec1, set_ptr->kappa(), device_vals);
                checkCudaErrors(cudaStreamSynchronize(set_ptr->stream));
            }
            { // _wilson_dslash_dag
                checkCudaErrors(cudaStreamSynchronize(set_ptr->stream));
    ...
    else if (data_type == _LAT_C128_)
    {
        ...
        // }
        {
            ...
            { // _wilson_dslash_no_dag
                ...
                _wilson_dslash.run_eo(device_vec0, fermion_in, gauge);
                _wilson_dslash.run_oe(device_vec1, device_vec0, gauge);
                cg_give_dest_o<double><<<set_ptr->gridDim, set_ptr->blockDim, 0,
                                         set_ptr->stream>>>(
                    device_vec2, fermion_in, device_vec1, set_ptr->kappa(), device_vals);
                checkCudaErrors(cudaStreamSynchronize(set_ptr->stream));
            }
