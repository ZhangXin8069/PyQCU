...
\item 缺口（E1，未填充）：MG-1 两侧都不可用；c128 的 QUDA 侧缺
  complex128 canonical null vector/QIO 资产；双 P100 的 QUDA 侧缺
...
12 个 side-case 两侧都不可用（原因见“局限”）。c128 与双 P100 列的
状态同样在“局限”中逐条说明，矩阵状态文件
\pathref{data/mg_matrix_full_v100_c64/matrix_state.jsonl} 保留了每个
单元的命令行、返回码与错误码。
...
所有有效单元的 full true residual 都低于 $10^{-6}$（gate
$5\times10^{-6}$），两侧都没有出现假收敛。
...
\item MG-1 层：两侧都无法给出可比较的数值。PyQCU strict fused
  ...
  替换实现。
\item c128 的 QUDA 侧需要 complex128 的 canonical null vector 与
  QIO 资产；若本轮未生成，则对应单元记为 blocked。
\item 双 P100 列需要把 QUDA 重新编译到 \code{sm\_60}：仓库中现有
  \code{quda-double-install} 只含 \code{sm\_70} cubin，P100 无法加载。
