...
\begin{frame}[t,shrink=5]{QCU ABI 桥接：P/R 与完整 33 点粗算子已逐项对齐}
  ...
  \end{alertblock}
  \source{实现：\codepath{_quda_multigrid.py}、\codepath{cpp/cuda/qcu/include/lattice_multigrid.h}；测试：\codepath{test_quda_multigrid.py}、\codepath{test_quda_transfer_cuda.py}、\codepath{examples/qcu/dev87/coarse_mpi_equiv.py}、\codepath{coarse_mpi_smoke.py}、\codepath{run_qcu_mg_mpi.py}；kernel：\codepath{cpp/cuda/qcu/src/multigrid.cu}。}
\end{frame}
...
\begin{frame}[t,shrink=5]{阶段结论：代数与 QCU 两 rank 小格系统验证已闭环，生产复现仍需三步}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \end{column}
  \end{columns}
  \source{源码：QUDA 四个核心文件；实现/测试：\codepath{_quda_multigrid.py}、\codepath{cpp/cuda/qcu/include/lattice_multigrid.h}、\codepath{examples/qcu/dev87/run_qcu_mg_mpi.py}。}
\end{frame}
