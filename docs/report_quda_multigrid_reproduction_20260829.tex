...
\begin{frame}[t,shrink=5]{QCU ABI 桥接：P/R 与完整 33 点粗算子已逐项对齐}
  ...
  \begin{alertblock}{规模边界}
    \scriptsize matrix-free transition 默认不触发逐列 materialize；大格应调用已有 \codepath{tools.build_stencil_mt} / \codepath{build_stencil_local}，再按上述槽位交给 CUDA。这里验证的是代数/ABI 等价，不宣称 setup 性能等价。
  \end{alertblock}
  \source{实现：\codepath{_quda_multigrid.py}；测试：\codepath{test_quda_multigrid.py}、\codepath{test_quda_transfer_cuda.py}；kernel：\codepath{cpp/cuda/qcu/src/multigrid.cu}。}
\end{frame}

\begin{frame}[t,shrink=5]{阶段结论：数学复现已闭环，系统复现仍需三步}
  \small
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.30\textwidth}
      \begin{block}{主张}
    ...
    \begin{column}{0.30\textwidth}
      \begin{block}{剩余最大风险}
        参考路径的精确 materialize 仍是逐点/逐列 Python 实现；大格应走批量/局部 stencil builder。生产级 MPI 通信、完整 setup 和性能尚未形成证据。
      \end{block}
      \begin{block}{旧实现定位}
    ...
    \begin{column}{0.30\textwidth}
      \begin{block}{下一步可验收产物}
        \scriptsize
        \begin{minipage}{0.97\linewidth}
          1. 多 rank coarse halo/ghost 与边界回归；\\
          2. 完整 QUDA 风格 null-vector setup（CG/CA-CG/Krylov）；\\
          3. 大格 residual、setup 成本与性能表。
        \end{minipage}
      \end{block}
...
\begin{frame}[t]{附录：本次交付清单}
  ...
  \begin{tabular}{@{}l@{}}
    ...
    \textbf{测试：}\quad \codepath{examples/pyqcu/test_quda_multigrid.py}（13 项 CPU 代数/求解器回归）\\
    \textbf{QCU 桥接：}\quad \codepath{examples/qcu/dev87/test_quda_transfer_cuda.py}（transfer/stencil CUDA 等价）\\
