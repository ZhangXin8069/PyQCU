...
\begin{frame}[t]{结论先行：QUDA 的代数主线已落到 PyQCU 参考实现}
  ...
  \end{tabular}
  \source{实现：\codepath{pyqcu/solver/_quda_multigrid.py:218-1689}；测试：\codepath{examples/pyqcu/test_quda_multigrid.py:76-389}；本次命令：\codepath{pytest -q examples/pyqcu}。}
\end{frame}
...
\begin{frame}[t]{代数验证结果：核心恒等式与求解 residual 均通过}
  ...
  \end{alertblock}
  \source{命令（在 \codepath{examples/pyqcu}）：\codepath{pytest -q test_quda_multigrid.py}；复核：\codepath{QudaMultigrid.diagnostics()}；测试：\codepath{test_quda_multigrid.py:76-389}。}
\end{frame}
...
\begin{frame}[t]{QCU MPI 多拓扑 full MG 已通过，真残差保持在 $4.1\times10^{-7}$}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \end{column}
  \end{columns}
  \source{命令：\codepath{examples/qcu/dev87/coarse_mpi_equiv.py}、\codepath{coarse_mpi_smoke.py}、\codepath{run_qcu_mg_mpi.py}；实现：\codepath{cpp/cuda/qcu/include/lattice_multigrid.h}。}
\end{frame}
...
\begin{frame}[t,shrink=5]{阶段结论：Python 参考路径已闭环，生产级 setup 与性能仍是边界}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \end{column}
  \end{columns}
  \source{源码：QUDA 四个核心文件；实现/测试：\codepath{_quda_multigrid.py}、\codepath{examples/pyqcu/test_quda_multigrid.py}、\codepath{cpp/cuda/qcu/include/lattice_multigrid.h}。}
\end{frame}
...
\begin{frame}[t]{null-vector setup：QUDA 两种语义与五类 solver 已可验证}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \end{column}
  \end{columns}
  \source{实现：\codepath{_quda_multigrid.py:1451-1626}。\\[-0.1em]测试：\codepath{test_quda_multigrid.py:304-389}。\\[-0.1em]命令：\codepath{pytest -q examples/pyqcu}。}
\end{frame}
...
\begin{frame}[t]{附录：本次交付清单}
  ...
  \begin{tabular}{@{}l@{}}
    ...
    \textbf{测试：}\quad \codepath{examples/pyqcu/test_quda_multigrid.py}（21 项 CPU 代数/setup/求解器回归）\\
    \textbf{QCU/MPI：}\quad \codepath{examples/qcu/dev87/test_quda_transfer_cuda.py}\\
    \hspace{4.8em}\codepath{examples/qcu/dev87/coarse_mpi_equiv.py}、\codepath{examples/qcu/dev87/coarse_mpi_smoke.py}\\
    \hspace{4.8em}\codepath{examples/qcu/dev87/run_qcu_mg_mpi.py}\\
