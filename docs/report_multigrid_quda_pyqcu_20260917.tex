...
c128 三层 $16\cdot32\cdot32\cdot48$ 的 $C=1$ 冷 setup 在修正
\code{checkCudaErrors} 双重求值错误后，运行约 $36.5$ 分钟后在
\code{lattice\_set.h:414} 的 \code{cudaMallocAsync} 真实返回
\code{CUDA error 0002 "out of memory"}；因此它在 32 GiB V100 上的
容量边界是明确的，而不是旧日志中的伪 \code{error 0000}。现在的
extended builder 使用
...
\pathref{data/mg_matrix_20260916_round2/formal-c64-l3/benchmark-formal-sm70-recheck.json} &
...
\pathref{data/mg_matrix_20260916_round2/formal-c128-l3/benchmark-formal-sm70-recheck.json} &
...
solve 真残差通过。\\
\pathref{data/mg_matrix_20260916_round2/formal-c128-l3/pyqcu-double-c1-parity-fixed.json} &
c128 大格 $C=1$ 冷 setup 的真实容量失败：\code{cudaMallocAsync} 返回
\code{error 0002 "out of memory"}，不是伪 \code{0000}。\\
...
\begin{enumerate}
  ...
  \item QUDA double MG 与 PyQCU double MG 均已推进到 c128 小/中格点
        ...
        $7.8417\times$（首次 $8.3507\times$），但冷 $C=1$ setup 仍未完成，
        且 QUDA plain
