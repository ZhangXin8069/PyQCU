...
探索配置在 32 GiB V100 上双侧已完成 smoke：MG steady 为
PyQCU $2.06395$ s、QUDA $17.35793$ s，探索比值 $8.4100\times$；
formal 仍锁定 $C=1$ 与 1 GiB setup cap，因此该大格点不进入正式
speedup 表。
\end{abstract}
...
c128 三层 $16\cdot32\cdot32\cdot48$ 的早期 $C=1$ 全 double 尝试曾
在 32 GiB V100 上 OOM 或超过 30 分钟，QUDA 也在 double coarse-link
setup 请求 75 MiB 时失败。当前 32 GiB 环境已经可以运行双侧全 double
smoke：PyQCU 使用 $C=4$、$K=16$、CPU canonical-block staging、
null-basis offload，并在 runtime seal 后调用 \code{empty\_cache()}；
QUDA 使用同参数协议和 double coarse fields。PyQCU 两侧 setup 统计为
$723.72$ s 与 $250.35$ s，QUDA setup 为 $630.74$ s。两者 solve 均为
20/60 次外迭代，相对真残差分别为 $1.7880\times10^{-8}$ 与
$1.7924\times10^{-8}$，因此两侧都通过独立真残差门。

该探索点的 MG steady 为 PyQCU $2.06395$ s、QUDA $17.35793$ s，
比值 $8.4100\times$；但 profile 标记为 \code{smoke-pass}，不是 formal，
不能替换主表。同侧 plain BiCGStab 为 PyQCU $3.25918$ s、QUDA
$0.97771$ s；QUDA BiCGStab 比 PyQCU MG 快约 $2.11\times$，再次说明
该探索比值只能解释为 MG 路径差异。
...
\begin{enumerate}
  ...
  \item QUDA double MG 与 PyQCU double MG 均已推进到 c128 小/中格点
        正式可运行；大格三层全 double 已得到双侧 smoke-pass，探索 MG
        比值为 $8.4100\times$，但 formal $C=1$ 仍未完成，不能把 smoke
        比值冒充正式结论。
