...
当前可信的大格 c64 三层主结果为 PyQCU $0.654446$ s、QUDA $1.299921$ s，
加速比 $1.9863\times$，外迭代数 $14/39$，真残差分别为
$4.90135\times10^{-7}$ 与 $7.32826\times10^{-7}$。c128 三层在
$16^3\cdot16$ 上得到 PyQCU $0.365517$ s、QUDA $1.290496$ s，
加速比 $3.5306\times$，外迭代数 $44/87$。c128 三层
$16\cdot32\cdot32\cdot48$ 的全 double setup 在 32 GiB V100 上双侧 OOM，
因此不报告跨库大格 c128 加速比。
\end{abstract}
...
$16\cdot32\cdot32\cdot48$ / 3 / c64 & 0.654446 & 1.299921 & 1.9863 & 14 / 39 \\
$8^3\cdot16$ / 3 / c128 & 0.077162 & 0.975873 & 12.6471 & 14 / 46 \\
$16^3\cdot16$ / 3 / c128 & 0.365517 & 1.290496 & 3.5306 & 44 / 87 \\
\bottomrule
\end{tabular}
\end{table}
...
$1.1226\times$，三层为 $1.9863\times$。这不是单点偶然值，而是层数
消融：增加第三层后 PyQCU 时间从 $1.944663$ s 降至 $0.654446$ s，
QUDA 从 $2.183002$ s 降至 $1.299921$ s，两侧均从更深粗化中获益，
但 PyQCU 的降幅更大。
...
\centering
\small
\caption{大格 c64 三层：MG 与同侧 plain BiCGStab median。}
\label{tab:bicg}
\begin{tabular}{@{}L{0.23\textwidth}rr@{}}
\toprule
实现 & MG (s) & BiCGStab (s) \\
\midrule
PyQCU & 0.654446 & 1.709560 \\
QUDA & 1.299921 & 0.329646 \\
\bottomrule
\end{tabular}
\end{table}

表中 QUDA 的 MG 慢于其 plain BiCGStab，说明 aligned 的“一次递归”配置
虽然数学上对齐，却不是 QUDA 在该格点上的绝对最快配置；library
敏感性表则给出了另一侧证据。因此本文同时报告两套策略，但主结论仍使用
aligned，因为它排除了“QUDA 多执行内层粗迭代而 PyQCU 只执行一次递归”的
口径差。
...
c128 三层 $16\cdot32\cdot32\cdot48$ 的全 double hierarchy setup 在
32 GiB V100 上双侧 OOM。PyQCU 最初在 block orthogonalization 请求额外
864 MiB 时失败；将 CGS 从三矩阵副本降为两缓冲后，该点可通过，但随后在
\code{to\_qcu\_blocked()} 导出 blocked basis 时仍 OOM。进一步把 blocked
布局改为正交化时直接分配的主存储后，导出已成为零拷贝；setup 继续推进到
Galerkin 粗块分配，最终仍因整套 double hierarchy 超出 32 GiB 而 OOM。
这说明限制不是单个临时数组，而是 fine fields、blocked basis、粗块与
coarse assets 的联合驻留。QUDA 则在 double coarse-link setup 请求
75 MiB 时失败。把 PyQCU 层数从三层降到二层后仍在同一个
\code{build\_strict\_galerkin\_colored()} 粗块分配点 OOM，进一步说明
大格点不可达的主导因素是全 double 的 fine/coarse 精度预算，而不是额外
第三层本身的体积。
...
batch 降到 1 后，仍在随后的 site-matrix contraction 处 OOM。这作为后续
strict mixed-precision 实现的入口，而不是已完成结论。
...
\pathref{data/mg_matrix_20260916_round2/formal-c64-l3/benchmark.json} &
当前 c64 大格三层 aligned 五次正式记录。\\
...
c64 小格三层 formal。\\
\pathref{data/mg_matrix_20260916_round2/formal-c128-l3-small/benchmark.json} &
c128 小格三层 formal，double MG 路径。\\
\pathref{data/mg_matrix_20260916_round2/formal-c128-l3-medium/benchmark.json} &
c128 中格三层 formal，double MG 路径。\\
...
\begin{enumerate}
  \item 旧 c64 $4.3817\times$ 来自未对齐内层预算，已撤回；当前大格 c64
        三层 aligned 主结果为 $1.9863\times$，五次样本和真残差均通过。
  \item QUDA double MG 已从“不可用”推进到 c128 小/中格点真实可运行，
        但全 double 大格三层受 32 GiB V100 显存限制，不能伪造跨库比值。
