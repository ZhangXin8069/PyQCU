...
\begin{frame}[t]{CUDA 单 rank 生产回归通过：两层有收益，三层保持正确但代价更高}
  ...
  \source{命令：\codepath{source ./env.sh \&\& bash ./build.sh}；
    \codepath{python examples/qcu/conftest.schur.multigrid.py}；
...
\begin{frame}[t]{本轮代数证据：布局、Schur、Galerkin 与求解均通过}
  ...
  \source{命令：定向测试与 \codepath{examples/pyqcu} 均 26 passed；
    数值：compact 直测与 \codepath{examples/qcu/conftest.schur.multigrid.py}。}
\end{frame}
...
\begin{frame}[t]{当前限制与风险：代数闭环不等于生产性能闭环}
  ...
  \source{实现边界：\codepath{_quda_multigrid.py:748-910,2145-2163}；
    QCU：\codepath{examples/qcu/conftest.schur.multigrid.py}；
    历史证据：\codepath{examples/qcu/dev87/}。}
\end{frame}
...
\begin{frame}[t]{阶段结论与后续：单 rank QCU 已闭环，下一步进入 MPI 与 setup 优化}
  ...
  \source{行动依据：代数测试、\codepath{logs/schur_mg_results.json} 与
    \codepath{examples/qcu/conftest.schur.multigrid.py}；MPI/资源节点待排期。}
\end{frame}
...
\begin{frame}[t]{附录：本轮验证命令与交付闸门}
  ...
  \begin{tabular}{@{}l@{}}
    ...
    \texttt{\$ python examples/qcu/conftest.schur.multigrid.py}
