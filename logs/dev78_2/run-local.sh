#!/usr/bin/env bash
# dev78_2 —— 多线程版（一线程一卡）CUDA C++ MultiGrid 求解器测试套件运行脚本。
#
...
#       → budget(16G P100 / 32G V100) → conv（全求解器迭代残差收集+图）→ 归档收敛日志。
#
# 数据持久化全部 h5py（main.py 内 save_dict_h5/load_dict_h5，独立 File 句柄
# 多线程安全）；PNG/TeX 为图表展示产物。
#
# 用法：
#   bash logs/dev78_2/run-local.sh            # 实际执行
#   bash logs/dev78_2/run-local.sh --dry-run  # 只打印命令
set -uo pipefail

REPO="${HOME}/PyQCU"
WORK="$REPO/logs/dev78_2"
MAIN="$WORK/main.py"
TS=$(date +%Y%m%d-%H%M%S)
...
{
echo "=== dev78_2 run-local @ $(date '+%Y-%m-%d %H:%M:%S') ==="
echo "VDIR=$VDIR"
...
# Step 8 全求解器迭代残差收集 + 残差图（MG 实测 CONVERGENCE_HISTORY + ref 复现）
step "Step 8 conv（全求解器迭代残差历史）"
run 5400 python "$MAIN" conv
run 300 python "$MAIN" conv_plots

# Step 9 归档 C++ 收敛日志
step "Step 9 归档 clover_multigrid.log"
