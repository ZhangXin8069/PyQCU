#!/usr/bin/env python3
"""dev78_2 补充图表：收敛图（conv_*）+ 显存图（vram）+ 日志分析图。
...
if conv:
    ...
    ax.set_title('dev78_2: convergence (BiStabCG, log scale)')
    ax.legend(fontsize=8)
    _style(ax)
    fig.savefig(os.path.join(WORK, 'dev78_2_conv_bench.png'), dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"wrote {WORK}/dev78_2_conv_bench.png ({len(conv)} configs)")
...
for tag, fn in [('16g', os.path.join(WORK, 'dev78_2_budget_16g.h5')),
                ('32g', os.path.join(WORK, 'dev78_2_budget_32g.h5'))]:
    b = load_budget(fn)
    if not b:
    ...
    ax.set_title(f'dev78_2: VRAM budget ({tag} card)')
    ax.legend()
    _style(ax)
    fig.savefig(os.path.join(WORK, f'dev78_2_vram_{tag}.png'), dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"wrote {WORK}/dev78_2_vram_{tag}.png")
...
if prof:
    ...
    ax.set_title('dev78_2: PROF_SECTIONS breakdown')
    ax.legend(fontsize=8)
    _style(ax)
    fig.savefig(os.path.join(WORK, 'dev78_2_prof.png'), dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"wrote {WORK}/dev78_2_prof.png ({len(prof)} runs)")
