#!/usr/bin/env python3
"""test11 —— dev74* 整合测试套件（单文件 main.py）。

背景：dev74（大格子 + 资源统计 + 多线程构建）与 dev74_1（服务器加速比 >1.5
验证）两套脚本整合为单一入口；工作目录固定为 logs/test11/，全部输出
（json/png/tex/收敛日志归档）落在该目录。

子命令：
  clean        干净测量（独立进程交叉计时 + 资源统计）→ test11_clean_*.json
  bench        批量基准（--mode local|server，预算自动跳过超限配置）
  verify       正确性验证（gauge/解/null_vecs/CudaSchurOp 对照）
  sweep        参数扫描（r/ct/cmi/levels × speedup）→ test11_sweep.json
  check        加速比断言（--gate 1.5 默认，exit 0/1/2）
  budget       显存/内存/磁盘预算表（--vram 16|32，默认 16GB）
  collect      汇总 → test11_results.json
  mktable      LaTeX 表 → test11_tbl_*.tex
  plots        dev74 风格图 → test11_*.png
  plots1       dev74_1 风格图（作图范围与 dev73_5 一致）→ test11_1_*.png
  layout_test  C++ Schur 算子输入布局对照实验
  stencil_mt   多线程 stencil build 对照验证

用法（PyQCU 根下）：
  source ./env.sh && python logs/test11/main.py <subcommand> [options]
...
def make_cuda_schur_ops(av, g, ce, coo, cei, coi, n=1, verbose=False):
    ...
    if verbose:
        print(f"[test11] created {n} CudaSchurOp set_index="
...
def fit_from_bench():
    """从 test11_bench.json 实测（peak_vram_mb, V）线性拟合 α/β。"""
    path = os.path.join(WORKDIR, "test11_bench.json")
...
def cmd_clean(args):
    ...
    label = (f"L{'x'.join(map(str,[Lx,Ly,Lz,Lt]))}_{args.prec}_"
             ...
             f"_{args.build}")
    res["label"] = label
    out_path = os.path.join(WORKDIR, f"test11_clean_{label}.json")
...
def cmd_verify(args):
    ...
    path = os.path.join(WORKDIR,
                        f"test11_verify_{'x'.join(map(str, lat_fine))}_{args.prec}.json")
...
def cmd_sweep(args):
    ...
    for i, c in enumerate(cfgs):
        ...
        p = os.path.join(WORKDIR, f"test11_clean_{label}.json")
    ...
    out_path = os.path.join(WORKDIR, "test11_sweep.json")
    with open(out_path, "w") as f:
...
def load_all(files):
    """读取显式指定的 json 文件；缺省读 test11_sweep.json。"""
    entries = []
    seen = set()
    if files:
        for p in files:
        ...
        p = os.path.join(WORKDIR, "test11_sweep.json")
        if os.path.exists(p):
            with open(p) as f:
...
def cmd_check(args):
    ...
    if not entries:
        print(f"[{args.label}] NO DATA — 请先运行 "
              f"python logs/test11/main.py sweep / clean")
        sys.exit(2)
...
def cmd_budget(args):
    ...
    if args.fit:
        ...
        if f:
            ...
            print(f"[fit] alpha={alpha:.2f} KB/V  beta={beta:.0f} MB "
                  f"(从 test11_bench.json 实测)")
        else:
            print("[fit] test11_bench.json 无实测数据，使用默认系数")
    ...
    out = os.path.join(WORKDIR, f"test11_budget_{args.mode}_{vram}g.json")
...
def cmd_collect(args):
    bench_path = os.path.join(WORKDIR, "test11_bench.json")
    bench = json.load(open(bench_path)) if os.path.exists(bench_path) else {"results": []}
    clean_files = sorted(glob.glob(os.path.join(WORKDIR, "test11_clean_L*.json")))
    ...
    verify = {}
    for vf in sorted(glob.glob(os.path.join(WORKDIR, "test11_verify_*.json"))):
        v = json.load(open(vf))
        key = (tuple(v["lattice"]), v["precision"])
        verify[str(key)] = v

    out_path = os.path.join(WORKDIR, "test11_results.json")
    with open(out_path, "w") as f:
...
def cmd_mktable(args):
    res_path = os.path.join(WORKDIR, "test11_results.json")
    ...
    with open(os.path.join(WORKDIR, "test11_tbl_main.tex"), "w") as f:
        f.write("% test11 —— 性能结果表\n")
    ...
    with open(os.path.join(WORKDIR, "test11_tbl_res.tex"), "w") as f:
        f.write("% test11 —— 资源占用统计（实测）\n")
        f.write("\\begin{tabular}{lcccccc}\n\\hline\n")
    ...
    bp = os.path.join(WORKDIR, f"test11_budget_{args.mode}_{args.vram}g.json")
    if os.path.exists(bp):
        budget = json.load(open(bp))
        with open(os.path.join(WORKDIR, "test11_tbl_budget.tex"), "w") as f:
            f.write(f"% test11 —— 服务器大格子预算预测（{args.vram}G 显存档，校准系数）\n")
            ...
            for row in budget["rows"]:
                ...
                f.write(f"\\texttt{{{L}}} & {row['V']} & "
                        ...
                        f"{row['vram_frac']:.2f} \\\\\n")
            f.write("\\hline\n\\end{tabular}\n")
    print(f"wrote test11_tbl_*.tex 到 {WORKDIR}")
...
def cmd_plots(args):
    ...
    plt.plot(V, sp, "o-", label="test11 实测")
    ...
    plt.title(f"test11: speedup vs V (vram={args.vram}G)")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(WORKDIR, "test11_speedup.png"), dpi=130)
    ...
    plt.title(f"test11: peak VRAM vs V (vram={args.vram}G)")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(WORKDIR, "test11_vram.png"), dpi=130)
    ...
    plt.title("test11: solve time")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(WORKDIR, "test11_time.png"), dpi=130)
    ...
    for r in results:
        ...
        plt.title(f"test11: {r['label']}")
        plt.legend()
        plt.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(WORKDIR,
                                 f"test11_conv_{r['label'].split('_')[0]}.png"),
                    dpi=130)

    bp = os.path.join(WORKDIR, f"test11_budget_server_{args.vram}g.json")
    if os.path.exists(bp):
        ...
        plt.ylabel("VRAM (GB)")
        plt.title(f"test11: server lattice budget ({args.vram}G 档)")
        plt.legend()
        plt.grid(alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(WORKDIR, "test11_budget.png"), dpi=130)

    nfig = len([f for f in os.listdir(WORKDIR)
                if f.startswith("test11") and f.endswith(".png")])
    print(f"wrote test11_*.png ({nfig} figures) → {WORKDIR}")
...
def _plot1_conv(results):
    ...
    for (lat, prec), rs in groups.items():
        ...
        fig.tight_layout()
        _fig_save(fig, f"test11_1_conv_{lat_f}_{prec}.png")
...
def _plot1_hotspot(results):
    ...
    fig.tight_layout()
    _fig_save(fig, "test11_1_hotspot.png")
...
def _plot1_speedup(results):
    ...
    fig.tight_layout()
    _fig_save(fig, "test11_1_speedup.png")
...
def _plot1_time(results):
    ...
    ax.legend(fontsize=8, frameon=False, loc="lower right")
    fig.tight_layout()
    _fig_save(fig, "test11_1_time.png")
...
def _plot1_sweep_curves(results, lattice):
    ...
    for ax, key, xlabel, title in [
        ...
        ax.grid(False, axis="x")
    fig.tight_layout()
    _fig_save(fig, "test11_1_sweep.png")
...
def cmd_stencil_mt(args):
    ...
    with open(os.path.join(WORKDIR, "test11_stencil_mt.json"), "w") as f:
        json.dump(res, f, indent=2)
    print(f"wrote {WORKDIR}/test11_stencil_mt.json")
...
# ----------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="test11 —— dev74* 整合测试套件")
    ...
    p.add_argument("--gate", type=float, default=1.5)
    p.add_argument("--label", default="test11 check")
    ...
    p = sub.add_parser("collect", help="汇总 → test11_results.json")
    p.set_defaults(func=cmd_collect)

    p = sub.add_parser("mktable", help="LaTeX 表 → test11_tbl_*.tex")
    p.add_argument("--mode", default="server", choices=["local", "server"])
    p.add_argument("--vram", type=int, default=16)
    p.set_defaults(func=cmd_mktable)

    p = sub.add_parser("plots", help="dev74 风格图 → test11_*.png")
    p.add_argument("--vram", type=int, default=16)
    p.set_defaults(func=cmd_plots)

    p = sub.add_parser("plots1", help="dev74_1 风格图（dev73_5 范围）→ test11_1_*.png")
    p.add_argument("--file", default=os.path.join(WORKDIR, "test11_sweep.json"))
