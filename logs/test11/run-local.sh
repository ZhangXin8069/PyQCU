#!/usr/bin/env bash
# test11 —— 本地运行脚本（RTX 4060 8GB 小卡验证）。
#
# 流程：verify → clean 8x8x8x16 → bench local → sweep 8x8x8x16
#       → collect/mktable/plots/plots1 → 归档收敛日志。
# 特点：
#   * 完整终端输出（tee 归档到 logs/test11/run-local-<ts>.log）
#   * 每步 timeout 防卡壳；单步失败仅记录并继续（GRID 保持轻量）
#
# 用法：
#   bash logs/test11/run-local.sh            # 实际执行
#   bash logs/test11/run-local.sh --dry-run  # 只打印命令
set -uo pipefail
...
echo "=== test11 local runner start $(date) ==="
