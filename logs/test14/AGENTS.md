...
| 数据持久化 | json（test12_results.json / env.json 等） | **全部 h5py**（env.h5 / test14_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄多线程安全，参考 pyqcu/tools/_io.py） |
...
├── test14_report.md  收尾结果报告（2026-08-15；本次工作汇总）
...
`--outdir` 公共参数（子命令前后皆可）；未指定读 `TEST14_OUTDIR` 环境变量，
再默认 `logs/test14/`。每次调用自动在输出目录写 `env.h5`。
