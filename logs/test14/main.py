...
背景：test12 测试单线程 C++ MultiGrid；test14 测试 `MultiGpuMultigrid`
...
子命令：
  ...
  clean         干净测量（独立进程交叉计时 + RSS）→ test14_clean_*.h5
  bench         批量基准（P100×2 多线程组 + V100 单线程大格子组）→ test14_bench.h5
  sweep         参数扫描（r/ct/cmi/levels × speedup，P100×2）→ test14_sweep.h5
  check         加速比断言（--gate 1.5 默认，exit 0/1/2）
  budget        显存/内存预算表（--vram 16 P100 档 / 32 V100 档）→ test14_budget_*.h5
  collect       汇总 → test14_results.h5
  mktable       LaTeX 表 → test14_tbl_*.tex
  plots         PNG 图 → test14_*.png

用法（PyQCU 根下）：
  source ./env.sh && python logs/test14/main.py <subcommand> [options]
...
# 路径与公共工具
# ----------------------------------------------------------------------
_HERE = os.path.dirname(os.path.abspath(__file__))            # logs/test14
REPO = os.path.abspath(os.path.join(_HERE, os.pardir, os.pardir))

# 输出目录重定向：优先级 --outdir > 环境变量 TEST14_OUTDIR > 默认 logs/test14
WORKDIR = os.environ.get("TEST14_OUTDIR", _HERE)
...
def cmd_verify(args):
    ...
    out = os.path.join(WORKDIR, "test14_verify.h5")
...
def cmd_clean(args):
    ...
    res['label'] = label
    out = os.path.join(WORKDIR, f"test14_clean_{label}.h5")
    save_dict_h5(out, res)
...
def cmd_bench(args):
    ...
    out = os.path.join(WORKDIR, "test14_bench.h5")
            ...
            for ct in cts:
                for cmi in cmis:
                    ...
                    print(f"sweep L{levels} r{r} ct{ct:.0e} cmi{cmi}: "
                          ...
                          flush=True)
    out = os.path.join(WORKDIR, "test14_sweep.h5")
...
def cmd_budget(args):
    ...
    for lat, levels in args.lattices:
        ...
        rows.append({'lattice': [int(x) for x in lat], 'levels': levels,
                     ...
                     'fit': cold < 0.9 * vram})
    out = os.path.join(WORKDIR, f"test14_budget_{vram}g.h5")
...
def _load_clean_entries():
    entries = []
    for f in sorted(glob.glob(os.path.join(WORKDIR, "test14_clean_*.h5"))):
        try:
...
def _bench_entries():
    p = os.path.join(WORKDIR, "test14_bench.h5")
...
def _sweep_entries():
    p = os.path.join(WORKDIR, "test14_sweep.h5")
    if not os.path.exists(p):
        return []
    d = load_dict_h5(p)
    return _entries_list(d)
...
def cmd_collect(args):
    ...
    out = os.path.join(WORKDIR, "test14_results.h5")
...
def cmd_mktable(args):
    ...
    out = os.path.join(WORKDIR, "test14_tbl_bench.tex")
    ...
    if sweep:
        ...
        lines += ["\\hline", "\\end{tabular}"]
        out = os.path.join(WORKDIR, "test14_tbl_sweep.tex")
        with open(out, 'w') as f:
            f.write("\n".join(lines) + "\n")
        print(f"wrote {out}")
...
def cmd_plots(args):
    ...
    if bench:
        ...
        ax.set_title('test14: Multi-thread MG speedup (P100x2 / V100)')
        ax.axvline(1.0, color='gray', ls='--', lw=0.8)
        _style(ax)
        _fig_save(fig, "test14_speedup.png")
        # 时间对比
        fig, ax = plt.subplots(figsize=(9, 5))
        x = range(len(bs))
    ...
    if sweep:
        ...
        _style(ax)
        _fig_save(fig, "test14_sweep.png")
        ...
        ax.set_xticks([])
        ax.set_ylabel('speedup')
        ax.set_title('test14: parameter hotspot (sweep 8x8x8x16)')
        _style(ax)
        _fig_save(fig, "test14_hotspot.png")
...
def main():
    p = argparse.ArgumentParser(
        description="test14: multi-thread (1-thread-1-GPU) CUDA C++ "
                    "MultiGrid solver test suite (h5py-only persistence)")
    p.add_argument('--outdir', default=None, help='输出目录（默认 TEST14_OUTDIR / logs/test14）')
    ...
    sp.add_argument('--file', default='test14_sweep.h5')
