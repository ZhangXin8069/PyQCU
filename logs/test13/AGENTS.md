# AGENTS.md — logs/test13

test13 —— **多线程版（一线程一卡）CUDA C++ MultiGrid 求解器**测试套件，
...
| 数据持久化 | json（test12_results.json / env.json 等） | **全部 h5py**（env.h5 / test13_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄多线程安全，参考 pyqcu/tools/_io.py） |
...
`--outdir` 公共参数（子命令前后皆可）；未指定读 `TEST13_OUTDIR` 环境变量，
再默认 `logs/test13/`。每次调用自动在输出目录写 `env.h5`。
