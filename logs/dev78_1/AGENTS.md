# AGENTS.md — logs/test78_1

test78_1 —— **多线程版（一线程一卡）CUDA C++ MultiGrid 求解器**测试套件，
...
| 数据持久化 | json（test12_results.json / env.json 等） | **全部 h5py**（env.h5 / test78_1_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄多线程安全，参考 pyqcu/tools/_io.py） |
...
├── test78_1_report.md  收尾结果报告（2026-08-15；本次工作汇总）
├── docs/             analy 报告（analy_test78_1_*.tex/.pdf）
└── v<YYYYMMDDHHMM>/  每次运行生成的版本目录（同分钟重跑加 -<SS>）
    ├── run-local-<ts>.log              完整终端输出（tee 归档）
    ├── env.h5                          环境快照（h5py；GPU/软件/git/命令）
    ├── test78_1_verify.h5                正确性验证（一致性/独立问题/V100/h5py IO）
    ├── test78_1_clean_L*.h5              干净测量（P100×2 多线程交叉计时 + RSS）
    ├── test78_1_bench.h5                 批量基准（P100×2 多线程组 + V100 单线程组）
    ├── test78_1_sweep.h5                 参数扫描（r/ct/cmi/levels × speedup）
    ├── test78_1_results.h5               collect 汇总（表/图输入）
    ├── test78_1_budget_*g.h5             显存/内存预算表（16G P100 / 32G V100 档）
    ├── test78_1_tbl_*.tex                LaTeX 表（bench/sweep）
    ├── test78_1_*.png                    图（加速比/耗时/参数扫描/热点）
...
cd /root/PyQCU
source ./env.sh
bash logs/test78_1/run-local.sh                # 实际执行；--dry-run 只打印命令
```
...
```bash
python logs/test78_1/main.py <subcommand> [options] [--outdir <dir>]
...
check   --gate 1.5 --file test78_1_sweep.h5         # 加速比断言（exit 0/1）
budget  --vram 16|32 [--lattices LxLyLzLt ...]    # 显存/内存预算表（默认 16G P100 档，32G V100 档）
collect | mktable | plots                         # 汇总 h5 / LaTeX 表 / PNG 图
```

`--outdir` 公共参数（子命令前后皆可）；未指定读 `TEST78_1_OUTDIR` 环境变量，
再默认 `logs/test78_1/`。每次调用自动在输出目录写 `env.h5`。
...
- **粗算子构建加速**（2026-08-15 test78_1 优化，pyqcu/tools/_multigrid.py +
...
`diff` 两个版本的 `test78_1_results.h5`（h5diff 或 load_dict_h5）直查差异。
