# AGENTS.md — logs/test78

test78 —— **多线程版（一线程一卡）CUDA C++ MultiGrid 求解器**测试套件，
...
| 数据持久化 | json（test12_results.json / env.json 等） | **全部 h5py**（env.h5 / test78_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄多线程安全，参考 pyqcu/tools/_io.py） |
...
├── test78_report.md  收尾结果报告（2026-08-15；本次工作汇总）
...
`--outdir` 公共参数（子命令前后皆可）；未指定读 `TEST78_OUTDIR` 环境变量，
再默认 `logs/test78/`。每次调用自动在输出目录写 `env.h5`。
...
- **粗算子构建加速**（2026-08-15 test78 优化，pyqcu/tools/_multigrid.py +
