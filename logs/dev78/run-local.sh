#!/usr/bin/env bash
# test78 —— 多线程版（一线程一卡）CUDA C++ MultiGrid 求解器测试套件运行脚本。
#
...
#
# 用法：
#   bash logs/test78/run-local.sh            # 实际执行
#   bash logs/test78/run-local.sh --dry-run  # 只打印命令
set -uo pipefail

REPO="${HOME}/PyQCU"
WORK="$REPO/logs/test78"
MAIN="$WORK/main.py"
TS=$(date +%Y%m%d-%H%M%S)
...
{
echo "=== test78 run-local @ $(date '+%Y-%m-%d %H:%M:%S') ==="
echo "VDIR=$VDIR"
