...
```bash
python examples/qcu/dev74/mg_dev74_1_sweep.py --lattice 8 16 16 16 --pairs 3
```

自动扫描 9 个配置（r=5/10/20、ct=1e2/1e3/1e5、cmi=15/50/200、2L/3L），
结果 `logs/dev74/dev74_1_sweep.json`。
...
```bash
python examples/qcu/dev74/mg_dev74_clean.py --lattice 16 32 32 32 --prec c64 \
...
python examples/qcu/dev74/mg_dev74_clean.py --lattice 16 32 32 64 --prec c64 \
...
```bash
python examples/qcu/dev74/mg_dev74_1_check.py --gate 1.5 --label "final"
```

一键流程（推荐）：`RUN=1 bash examples/qcu/dev74/mg_dev74_1_server.sh`
...
服务器上产物（已归档于 `logs/dev74/`，scp 回本地即可）：
