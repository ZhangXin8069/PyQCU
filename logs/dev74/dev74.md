...
> LaTeX 版：`logs/dev74/dev74.tex` → `logs/dev74/dev74.pdf`。
...
`examples/qcu/dev74/mg_dev74_cluster.sh`（dry-run 默认；`RUN=1` 执行）。
...
### 5.2 CudaSchurOp（`examples/qcu/dev74/mg_dev74_dslash.py`）
...
### 5.3 多线程 stencil build（`examples/qcu/dev74/mg_dev74_stencil_mt.py`）
