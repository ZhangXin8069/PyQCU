# AGENTS.md — logs/test76

test76 —— **多线程版（一线程一卡）CUDA C++ MultiGrid 求解器**测试套件，
...
| 数据持久化 | json（test12_results.json / env.json 等） | **全部 h5py**（env.h5 / test76_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄多线程安全，参考 pyqcu/tools/_io.py） |
...
├── test76_report.md  收尾结果报告（2026-08-15；本次工作汇总）
...
`--outdir` 公共参数（子命令前后皆可）；未指定读 `TEST76_OUTDIR` 环境变量，
再默认 `logs/test76/`。每次调用自动在输出目录写 `env.h5`。
...
- **粗算子构建加速**（2026-08-15 test76 优化，pyqcu/tools/_multigrid.py +
...
- **粗层求解加速**（2026-08-16 dev76，cpp/cuda/qcu/include/lattice_clover_multigrid.h）：
  * 多 block 归约（coarse_dot_kernel_multi + coarse_dot_reduce_kernel）：粗层
    向量 ≥64K 元素时 grid-stride 多 block 归约 + 单 block 二次归约；小向量保留
    单 block。16x16x16x32 3L 中间粗层（196608 元素）coarse_solve 533ms → 42ms
    （约 10 倍）。
  * 大最粗层 fused 回退：coarse_solve_fused 仅对 vec_sz < 64K 的最粗层使用
    （cooperative grid.sync 开销 × 迭代数在大系统上反而更慢），≥64K 走普通
    多 block 迭代路径。
  * 已回退尝试：中间层全 fused（并行度不足）、CHECK_INTERVAL 延迟收敛检查
    （破坏 count_restart 递归校正）。
  * 实测：P100x2 8x8x8x16 2L 1.83→2.01-2.34、3L 2.05→2.14-2.16；
    V100 16x16x16x16 3L 1.19→1.12-1.33、16x16x16x32 3L 0.75→0.95-1.20。
