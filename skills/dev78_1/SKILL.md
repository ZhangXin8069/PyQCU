---
name: dev78_1
description: logs/dev78_1 目录的完整生成 skill：多线程版（一线程一卡）CUDA C++ MultiGrid 求解器测试套件 — 单文件 main.py 子命令入口 + 版本化产物目录 v<ts>/ + 全部 h5py 持久化，测试 MultiGpuMultigrid（pyqcu/cuda/_multi_gpu.py）相对多线程 BiStabCG 的正确性与加速比（P100×2 多线程 + V100 单线程大格子）。
---
# Skill: dev78_1

# dev78_1 — 多线程版（一线程一卡）CUDA C++ MultiGrid 求解器测试套件

生成 `logs/dev78_1/` 工作目录的完整指南。目标：测试**多线程版**
...
| 数据持久化 | json | **全部 h5py**（env.h5 / dev78_1_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄） |
...
├── AGENTS.md         复现与比对指南
├── dev78_1_report.md  收尾结果报告
├── docs/             analy 报告（analy_dev78_1_*.tex/.pdf）
└── v<YYYYMMDDHHMM>/  每次运行生成的版本目录（同分钟重跑加 -<SS>）
    ├── run-local-<ts>.log              完整终端输出（tee 归档）
    ├── env.h5                          环境快照（h5py；GPU/软件/git/命令）
    ├── dev78_1_verify.h5                正确性验证（一致性/独立问题/V100/h5py IO）
    ├── dev78_1_clean_L*.h5              干净测量（P100×2 多线程交叉计时 + RSS）
    ├── dev78_1_bench.h5                 批量基准（P100×2 多线程组 + V100 单线程组）
    ├── dev78_1_sweep.h5                 参数扫描（r/ct/cmi/levels × speedup）
    ├── dev78_1_results.h5               collect 汇总（表/图输入）
    ├── dev78_1_budget_*g.h5             显存/内存预算表（16G P100 / 32G V100 档）
    ├── dev78_1_tbl_*.tex                LaTeX 表（bench/sweep）
    ├── dev78_1_*.png                    图（加速比/耗时/参数扫描/热点）
...
cd /root/PyQCU
source ./env.sh
bash logs/dev78_1/run-local.sh                # 实际执行；--dry-run 只打印命令
```
...
```bash
python logs/dev78_1/main.py <subcommand> [options] [--outdir <dir>]
...
check   --gate 1.5 --file dev78_1_sweep.h5         # 加速比断言（exit 0/1）
budget  --vram 16|32 [--lattices LxLyLzLt ...]    # 显存/内存预算表（16G P100 / 32G V100 档）
collect | mktable | plots                         # 汇总 h5 / LaTeX 表 / PNG 图
```

`--outdir` 公共参数；未指定读 `TEST78_1_OUTDIR` 环境变量，再默认 `logs/dev78_1/`。
每次调用自动在输出目录写 `env.h5`。
...
`diff` 两个版本的 `dev78_1_results.h5`（h5diff 或 load_dict_h5）直查差异。

## 粗算子构建加速（2026-08-15 dev78_1 核心改进）

- **nv_tol=1e-2**（build_schur_levels/give_null_vecs_mt 默认）：null 向量 BiCGStab
  容差 5e-5 → 1e-2（5e-5 在 16x16x16x32 lv2 迭代爆炸 >34min 未完成；1e-2 分钟级）。
- **批量 stencil 探测**（batch_build=True 默认）：_probe_point_batch 固定 c_idx 一次
  批量全部 E 探针（_schur_matvec_batch 细层 / _stencil_matvec_batch 粗层，torch
  einsum）—— 8x8x8x16 lv1 135.6s → 3.3s（21x）。
- **批量 BiCGStab**（_bistabcg_batch）：null 向量 dof 右端一次批量迭代（复数安全
  除法，与逐场等价 err~1e-5）—— 16x16x16x32 lv2 40min+ → 83s。
- 实测 16x16x16x32 3L 完整构建 86s（原 1h+ 未完成）；缓存 key 含 `_t{nv_tol}`。
- 正确性回归：8x8x8x16 2L speedup=1.95 consistency=True；sweep 16/16≥1.5
  （test13 11/16），best 2.15 → 2.49。

## 中/大格子参数优化（2026-08-16 dev78 核心改进）

- **num_restart 参数**：8x16x16x16 2L/3L 用 r10、16x16x16x16 2L 用 r20
  （V-cycle 频率减半，中格子粗层求解是校正主导成本）。
- 实测：8x16x16x16 2L 0.33-0.69→1.05-1.22（3 倍）、3L 0.45-0.72→1.41；
  bench median 1.148→1.210、max 2.135→2.288；verify 全 PASS。
- 16x16x16x16 2L r20=0.74 仍 <1（层数固有）；fused 阈值 32K 更差已回退。

## dev78_1 完整复测（2026-08-16）

- 参考 logs/dev74 输出形式：全套图表（speedup/time/sweep/hotspot +
  make_extra_plots.py 补充 conv/vram/prof）+ 全部日志（verify/clean/bench/sweep
  独立 log + C++ 收敛日志）+ 详细报告（md + docs/analy tex/pdf）。
- 实测：bench median 1.422、max 2.271、sweep 14/16≥1.5（best 2.480）、
  verify 全 PASS；8x16x16x16 2L r10=1.48（r5 时 0.33-0.69）。
...
版本目录；AGENTS.md 为复现指南（见 `logs/dev78_1/AGENTS.md` 权威版本，
本 skill 保持同步）。
