...
## Wuppertal Gaussian Smearing (`_wuppertal.py`, 2026-08-24)

与 stout 同目录的高斯模糊。bug33/34 修复要点（`logs/fix-report-2026-08-24.md`）：

- wards 含 t 维时系数表须同步扩展为 8 邻居（只改 x/y/z 会漏 t 方向），否则常数场每步
  按 (1+2σ) 发散；**U=I 不动点判据**（单位规范场模糊后仍为单位场，<1e-4）是廉价黄金自检。
- MPI halo 交换每步须重做，且两次 Sendrecv 的 recvtag 必须区分方向——第二次错复用
  rank_plus 在 grid 维度=2 时周期退化死锁；边界切片缺方向轴仅在 7v6 等不对称网格显性崩溃。
- 黄金回归判据：np=2/4 常数源 rel≈5e-08（test16 三重不变量守护）。
