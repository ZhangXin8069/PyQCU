...
| `refer/` | 开发历史报告（dev71.*） |
| `logs/` | 按 tag 归档：`dev<N>/`、`stab<N>/`、`bug<N>/` 子目录（如 `dev73/`、`dev73/stab24/`、`dev74/`、`bug30/`；`logs/<tag>/**` 在 .gitignore 全豁免入库），根目录留 `fix-report-*.md`、`debug/`、`results/` 与共享缓存 `nullvec_cache/`；测试套件 `test11/`（历史版）、`test12/`（单线程版）、`test13/`（多线程版，当前版）与 `test14/`（多线程版+粗算子构建加速，产物归档 `testNN/v<ts>/`） |
