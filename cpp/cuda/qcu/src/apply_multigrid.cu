...
void applyMultigridCoarseDslashWideQcu(long long _fermion_out,
  ...
  void *hop_nn = (void *)_hop_nn;
  void *hop_diag = (void *)_hop_diag;
  void *set_ptrs = (void *)_set_ptrs;
  int *params = (int *)_params;
  int set_index = params[_SET_INDEX_];
  int data_type = params[_DATA_TYPE_];
  ...
  if (data_type == _LAT_C64_) {
    LatticeSet<float> *set_ptr =
        static_cast<LatticeSet<float> *>(
            (void *)(static_cast<long long *>(set_ptrs)[set_index]));
    LatticeMultigridCoarseDslashWide<float> _coarse_dslash;
    _coarse_dslash.give(set_ptr);
    _coarse_dslash.run(fermion_out, fermion_in, sitting, hop_nn, hop_diag,
                       E, X, Y, Z, Lt);
  } else if (data_type == _LAT_C128_) {
    LatticeSet<double> *set_ptr =
        static_cast<LatticeSet<double> *>(
            (void *)(static_cast<long long *>(set_ptrs)[set_index]));
    LatticeMultigridCoarseDslashWide<double> _coarse_dslash;
    _coarse_dslash.give(set_ptr);
    _coarse_dslash.run(fermion_out, fermion_in, sitting, hop_nn, hop_diag,
                       E, X, Y, Z, Lt);
  } else {
    printf("data_type error in applyMultigridCoarseDslashWideQcu\n");
  }
  cudaDeviceSynchronize();
}
