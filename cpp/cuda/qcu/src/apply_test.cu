...
void testCloverDslashQcu(long long _fermion_out, long long _fermion_in,
  ...
  if (data_type == _LAT_C64_) {
    ...
    void *clover;
    checkCudaErrors(cudaMallocAsync(&clover,
                                    (set_ptr->lat_4dim * _LAT_SCSC_) *
                                        sizeof(LatticeComplex<float>),
                                    set_ptr->stream));
    cudaError_t err;
    {
      // wilson dslash
    ...
    }
    {
      ...
      checkCudaErrors(err);
      printf(
          "make clover total time: (without malloc free memcpy) :%.9lf sec\n ",
          double(duration) / 1e9);
    }
    {
      // inverse clover
