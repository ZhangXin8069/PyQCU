...
__global__ void wilson_dslash(void *device_U, void *device_src,
  ...
  {   // x part
    { // x-1
      ...
      tmp_U = (origin_U + move + (_X_ * _EVEN_ODD_ + (1 - parity)) * lat_xyzt);
...
wilson_dslash_x_send(void *device_U, void *device_src, void *device_params,
  ...
  { // x+1
    ...
    // send in x-1 way
    tmp_U =
        (origin_U + (_X_ * _EVEN_ODD_ + (1 - parity)) * lat_xyzt); // even-odd
    give_u(U, tmp_U, lat_xyzt);
    get_src(src, origin_src, lat_xyzt);
    { // just tmp
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
...
wilson_dslash_y_send(void *device_U, void *device_src, void *device_params,
  ...
  { // y+1
    ...
    // send in y-1 way
    tmp_U =
        (origin_U + (_Y_ * _EVEN_ODD_ + (1 - parity)) * lat_xyzt); // even-odd
    give_u(U, tmp_U, lat_xyzt);
    get_src(src, origin_src, lat_xyzt);
    { // just tmp
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
...
wilson_dslash_z_send(void *device_U, void *device_src, void *device_params,
  ...
  { // z+1
    ...
    // send in z-1 way
    tmp_U =
        (origin_U + (_Z_ * _EVEN_ODD_ + (1 - parity)) * lat_xyzt); // even-odd
    give_u(U, tmp_U, lat_xyzt);
    get_src(src, origin_src, lat_xyzt);
    { // just tmp
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
...
wilson_dslash_t_send(void *device_U, void *device_src, void *device_params,
  ...
  { // t+1
    ...
    // send in t-1 way
    tmp_U =
        (origin_U + (_T_ * _EVEN_ODD_ + (1 - parity)) * lat_xyzt); // even-odd
    give_u(U, tmp_U, lat_xyzt);
    get_src(src, origin_src, lat_xyzt);
    { // just tmp
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
