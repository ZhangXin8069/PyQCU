...
__global__ void _sctzyx2tzyxsc(void *device_fermi, void *device__fermi,
  ...
  // fermion[0]._data.y); // test - bug10 refer to .log-bug10.txt
  for (int i = 0; i < _LAT_SC_; i++) {
...
void tzyxsc2sctzyx(void *fermion, LatticeSet<T> *set_ptr) {
  ...
  checkCudaErrors(cudaMallocAsync(
      &_fermion, set_ptr->lat_4dim_SC * sizeof(LatticeComplex<T>),
      set_ptr->stream));
  _tzyxsc2sctzyx<T>
  ...
  CUBLAS_CHECK(_cublasCopy<T>(set_ptr->cublasH,
                              set_ptr->lat_4dim_SC * _REAL_IMAG_, (T *)_fermion,
                              1, (T *)fermion, 1));
...
__global__ void _ccdptzyx2dptzyxcc(void *device_gauge, void *device__gauge,
  ...
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  LatticeComplex<T> *gauge =
      ((static_cast<LatticeComplex<T> *>(device_gauge)) + idx);
  LatticeComplex<T> *_gauge =
...
void dptzyxcc2ccdptzyx(void *gauge, LatticeSet<T> *set_ptr) {
  ...
  checkCudaErrors(cudaMallocAsync(
      ...
      set_ptr->stream));
  _dptzyxcc2ccdptzyx<T>
      <<<set_ptr->gridDim, set_ptr->blockDim, 0, set_ptr->stream>>>(
          gauge, _gauge, set_ptr->lat_4dim);
  CUBLAS_CHECK(_cublasCopy<T>(set_ptr->cublasH,
                              set_ptr->lat_4dim_DCC * _EVEN_ODD_ * _REAL_IMAG_,
...
void ptzyxsc2psctzyx(void *fermion, LatticeSet<T> *set_ptr) {
  ...
  _ptzyxsc2psctzyx<T>
      <<<set_ptr->gridDim, set_ptr->blockDim, 0, set_ptr->stream>>>(
          fermion, _fermion, set_ptr->lat_4dim);
  CUBLAS_CHECK(_cublasCopy<T>(set_ptr->cublasH,
                              set_ptr->lat_4dim_SC * _EVEN_ODD_ * _REAL_IMAG_,
...
_tzyxsc2sctzyx<double>(void *device_fermi, void *device__fermi, int lat_4dim);
template __global__ void
_sctzyx2tzyxsc<double>(void *device_fermi, void *device__fermi, int lat_4dim);
...
template __global__ void _dptzyxcc2ccdptzyx<double>(void *device_gauge,
                                                    void *device__gauge,
                                                    int lat_4dim);
template __global__ void _ccdptzyx2dptzyxcc<double>(void *device_gauge,
                                                    void *device__gauge,
                                                    int lat_4dim);
template void dptzyxcc2ccdptzyx<double>(void *gauge,
                                        LatticeSet<double> *set_ptr);
template void ccdptzyx2dptzyxcc<double>(void *gauge,
                                        LatticeSet<double> *set_ptr);
template __global__ void
_ptzyxsc2psctzyx<double>(void *device_fermi, void *device__fermi, int lat_4dim);
template __global__ void
_psctzyx2ptzyxsc<double>(void *device_fermi, void *device__fermi, int lat_4dim);
template void ptzyxsc2psctzyx<double>(void *fermion,
                                      LatticeSet<double> *set_ptr);
template void psctzyx2ptzyxsc<double>(void *fermion,
                                      LatticeSet<double> *set_ptr);
//@@@CUDA_TEMPLATE_FOR_DEVICE@@@
...
template __global__ void give_1custom<float>(void *device_vals,
                                             ...
                                             float imag);
template __global__ void
_tzyxsc2sctzyx<float>(void *device_fermi, void *device__fermi, int lat_4dim);
template __global__ void
_sctzyx2tzyxsc<float>(void *device_fermi, void *device__fermi, int lat_4dim);
...
template __global__ void _dptzyxcc2ccdptzyx<float>(void *device_gauge,
                                                   void *device__gauge,
                                                   int lat_4dim);
template __global__ void _ccdptzyx2dptzyxcc<float>(void *device_gauge,
                                                   void *device__gauge,
                                                   int lat_4dim);
...
template __global__ void
_ptzyxsc2psctzyx<float>(void *device_fermi, void *device__fermi, int lat_4dim);
template __global__ void
_psctzyx2ptzyxsc<float>(void *device_fermi, void *device__fermi, int lat_4dim);
