...
\newcommand{\source}[1]{\vspace{0.04em}\par\noindent\parbox[t]{0.96\linewidth}{\raggedright\tiny\color{muted}来源：#1\par}}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{源码确证}
        \begin{tabular}{@{}l@{}}
          ...
          \verified\quad UV/VUV 装配粗算子；\\
          \verified\quad `verify()' 比较 $RDP$ 与 native apply；
        \end{tabular}
      \end{block}
...
\begin{frame}[t]{问题的第一性原理：粗空间要近似难解的低频子空间}
  ...
  \begin{center}
    \begin{tikzpicture}[node distance=0.25cm and 0.35cm]
      \node[phase,minimum height=0.50cm,font=\tiny] (h) {局部高频误差};
      \node[phase,minimum height=0.50cm,font=\tiny,right=of h] (s) {smoother\(S\)};
      \node[phase,minimum height=0.50cm,font=\tiny,below=of h] (l) {长程低频误差};
      \node[phase,minimum height=0.50cm,font=\tiny,right=of l] (c) {粗空间\(P/R\)};
      \draw[arrow] (h)--(s);
      \draw[arrow] (l)--(c);
      \draw[relation] (s)--(c);
    \end{tikzpicture}
    ...
    \begin{column}{0.64\textwidth}
      \begin{block}{语言无关等价实现}
        ...
        \begin{tabular}{@{}l@{}}
          ...
          $\quad MG_{\ell+1}\leftarrow\text{setupLevel}(\ell+1,$ \\
          $\qquad D_{\ell+1},B_{\ell+1})$ \\
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.63\textwidth}
      \begin{block}{aggregate 路径}
        ...
        \begin{tabular}{@{}l@{}}
          ...
          $\textbf{P: }\psi_f(x)\leftarrow\sum_{c_c,j}V_j(x)\,\eta_c(a(x),s_c,c_c,j)$ \\
          $\textbf{R: }\eta_c(a)\leftarrow\sum_{x\in a}V(x)^\dagger\psi_f(x)$ \\
    ...
    \begin{column}{0.32\textwidth}
      ...
      \begin{block}{变体}
        \begin{tabular}{@{}l@{}}
          aggregate：真正使用 $V$；\\
          coarse-KD：固定 staggered\\
          KD 结构；\\
          optimized-KD：元数据/等体积\\
          映射，\\
          不能直接套用 aggregate 公式。
        \end{tabular}
      \end{block}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.66\textwidth}
      \begin{block}{Wilson/Clover 及其变体的装配伪代码}
        \footnotesize
        \begin{tabular}{@{}l@{}}
          $\text{exchangeGhost}(V,nFace)$ \\
          $\qquad nFace=1\;(\text{Wilson/Clover}),\;3\;(\text{ASQTAD})$ \\
          ...
          $\quad\textbf{if long link: }LV_\mu\leftarrow L_\mu V$ \\
          $\qquad Y_\mu\mathrel{+}=V^\dagger(LV_\mu)$ \\
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.47\textwidth}
      \begin{block}{wrapper 语义}
        \scriptsize
        \begin{tabular}{@{}l@{}}
          $\text{coarse solver}=\text{PreconditionedSolver}(\text{solver},$ \\
          $\qquad\text{MG/smoother})$ \\
          $\text{solver}\in\{\text{GCR},\text{CGNR/CGNE},\text{BiCGStabL},$\\
          $\qquad\text{CA variants},\ldots\}$ \\
          $\text{next level MG}\;\Rightarrow\;\text{one recursive}$\\
          $\qquad\text{preconditioner application}$ \\
    ...
    \begin{column}{0.47\textwidth}
      \begin{block}{当前快照实际支持的分支}
        \begin{tabular}{@{}l@{}}
          \texttt{VCYCLE}：中间层可直接复用 coarse MG；\\
          \texttt{RECURSIVE}：创建 solver wrapper；\\
          FCYCLE/WCYCLE 枚举存在，但本实现\\
          工厂未走通用分支。\\[0.2em]
...
\begin{frame}[t]{实现层：halo、checkerboard、低精度与 MMA 只改变“如何算”}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    ...
    \begin{column}{0.34\textwidth}
      {\scriptsize
      \begin{tabularx}{0.98\linewidth}{@{}>{\raggedright\arraybackslash}p{0.46\linewidth}>{\raggedright\arraybackslash}X@{}}
        \toprule
        开关/字段 & 影响 \\
        \midrule
        ...
        \begin{tabular}{@{}l@{}}\texttt{setup/dslash/}\\\texttt{transfer\_use\_}\\\texttt{mma}\end{tabular} & 三类 kernel 的 MMA 选择 \\
        fixed-point & \begin{tabular}{@{}l@{}}max/scale/\\convert 协议\end{tabular} \\
        \bottomrule
      \end{tabularx}}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T,totalwidth=\textwidth]
    \begin{column}{0.54\textwidth}
      ...
      \begin{tabular}{@{}l@{}}
        ...
        $\text{only then enable half/fixed-point/MMA/halo}$ \\
        $\text{optimizations}$
      \end{tabular}
    \end{column}
    \begin{column}{0.40\textwidth}
      ...
      \begin{tabularx}{\linewidth}{@{}lX@{}}
        ...
        P/R 与 map & \begin{tabular}[t]{@{}l@{}}\codepath{lib/transfer.cpp},\\\codepath{include/transfer.h}\end{tabular} \\
        block GS & \begin{tabular}[t]{@{}l@{}}\codepath{include/kernels/}\\\codepath{block_orthogonalize.cuh}\end{tabular} \\
        coarse operator & \codepath{lib/coarse_op.cuh} \\
        coarse Dirac/PC & \codepath{lib/dirac_coarse.cpp} \\
        API/参数/测试 & \begin{tabular}[t]{@{}l@{}}\codepath{lib/interface_quda.cpp},\\\codepath{include/quda.h},\codepath{tests/}\end{tabular} \\
        \bottomrule
      \end{tabularx}
    \end{column}
  \end{columns}
