---
name: test12
description: logs/test12 目录的完整生成 skill：dev74*（dev74 + dev74_1）整合测试套件 test11 的优化版 — 单文件 main.py 子命令入口 + 版本化产物目录 v<ts>/，测试 CUDA C++ MultiGrid 求解器性能（正确性/干净测量/参数扫描/大格子预算/加速比图表）。
---
# Skill: test12

# test12 — dev74* 整合测试套件（test11_1 优化版）

生成 `logs/test12/` 工作目录的完整指南。目标：测试 CUDA C++ 版 MultiGrid 求解器
性能（正确性 / 干净测量 / 参数扫描 / 大格子预算 / 加速比图表）。测试代码与运行脚本
**自包含**（main.py 不 import 任何 dev73*/dev74*/test11 模块，仅内联照抄）。

## 核心设计：版本化产物目录

| 方面 | test11（历史版） | test12（当前版） |
|---|---|---|
...
本套件由整合 dev74*/test11 而来：main.py 为单文件入口（内联照抄 dev74 系列脚本的
clean/bench/verify/sweep/collect/mktable/plots 逻辑），run-*.sh 按环境组织流程并
自动创建版本目录；AGENTS.md 为复现指南（见 `logs/test12/AGENTS.md` 权威版本，
本 skill 保持同步）。
