---
name: test76
description: logs/test76 目录的完整生成 skill：多线程版（一线程一卡）CUDA C++ MultiGrid 求解器测试套件 — 单文件 main.py 子命令入口 + 版本化产物目录 v<ts>/ + 全部 h5py 持久化，测试 MultiGpuMultigrid（pyqcu/cuda/_multi_gpu.py）相对多线程 BiStabCG 的正确性与加速比（P100×2 多线程 + V100 单线程大格子）。
---
# Skill: test76

# test76 — 多线程版（一线程一卡）CUDA C++ MultiGrid 求解器测试套件

生成 `logs/test76/` 工作目录的完整指南。目标：测试**多线程版**
...
| 数据持久化 | json | **全部 h5py**（env.h5 / test76_*.h5；save_dict_h5/load_dict_h5，独立 File 句柄） |
...
├── AGENTS.md         复现与比对指南
├── test76_report.md  收尾结果报告
├── docs/             analy 报告（analy_test76_*.tex/.pdf）
└── v<YYYYMMDDHHMM>/  每次运行生成的版本目录（同分钟重跑加 -<SS>）
    ├── run-local-<ts>.log              完整终端输出（tee 归档）
    ├── env.h5                          环境快照（h5py；GPU/软件/git/命令）
    ├── test76_verify.h5                正确性验证（一致性/独立问题/V100/h5py IO）
    ├── test76_clean_L*.h5              干净测量（P100×2 多线程交叉计时 + RSS）
    ├── test76_bench.h5                 批量基准（P100×2 多线程组 + V100 单线程组）
    ├── test76_sweep.h5                 参数扫描（r/ct/cmi/levels × speedup）
    ├── test76_results.h5               collect 汇总（表/图输入）
    ├── test76_budget_*g.h5             显存/内存预算表（16G P100 / 32G V100 档）
    ├── test76_tbl_*.tex                LaTeX 表（bench/sweep）
    ├── test76_*.png                    图（加速比/耗时/参数扫描/热点）
...
cd /root/PyQCU
source ./env.sh
bash logs/test76/run-local.sh                # 实际执行；--dry-run 只打印命令
```
...
```bash
python logs/test76/main.py <subcommand> [options] [--outdir <dir>]
...
`diff` 两个版本的 `test76_results.h5`（h5diff 或 load_dict_h5）直查差异。

## 粗算子构建加速（2026-08-15 test76 核心改进）
...
## 粗层求解加速（2026-08-16 dev76 核心改进）

- **多 block 归约**（lattice_clover_multigrid.h）：coarse_dot_kernel_multi +
  coarse_dot_reduce_kernel，粗层向量 ≥64K 元素时 grid-stride 多 block 归约；
  16x16x16x32 3L coarse_solve 533ms → 42ms（约 10 倍）。
- **大最粗层 fused 回退**：coarse_solve_fused 仅对 vec_sz < 64K 的最粗层
  （cooperative grid.sync 在大系统上反而慢）。
- 已回退：中间层全 fused、CHECK_INTERVAL 延迟收敛检查（破坏递归校正）。
- 实测：8x8x8x16 2L 1.83→2.01-2.34、16x16x16x16 3L 1.19→1.12-1.33、
  16x16x16x32 3L 0.75→0.95-1.20；verify 全 PASS。
...
版本目录；AGENTS.md 为复现指南（见 `logs/test76/AGENTS.md` 权威版本，
本 skill 保持同步）。
